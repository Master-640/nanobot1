#!/bin/bash
# 并发启动 15 个实验容器（跳过 Task1_Sales/deepseek_chat）

MODELS=("deepseek-chat" "qwen3-max-2026-01-23" "kimi-k2.5" "MiniMax-M1")
...
for TASK in "${TASKS[@]}"; do
    for MODEL in "${MODELS[@]}"; do
        # 跳过 Task1_Sales + deepseek-chat
        if [[ "$TASK" == "Task1_Sales" && "$MODEL" == "deepseek-chat" ]]; then
            echo "跳过: $TASK / $MODEL (已完成)"
            continue
        fi
