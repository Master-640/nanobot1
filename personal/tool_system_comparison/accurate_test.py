...
def run_task(task_func, task_name: str, token_analyzer: TokenAnalyzer) -> Dict:
    ...
    print(f"✅ 任务执行完成，实际执行时间: {task_end - task_start:.2f}秒")
...
def main():
    ...
    if not os.path.exists(token_file):
        print(f"❌ Token记录文件不存在: {token_file}")
        print("请确保token_usage.txt文件存在")
        return
    
    analyzer = TokenAnalyzer(token_file)
...
def generate_detailed_report(results: List[Dict]):
    ...
    # 打印表格
    headers = ["任务名称", "实际时间(s)", "计算时间(s)", "Token消耗", "开始Token", "结束Token", "工具调用", "代码长度", "描述"]
    
    print("\n" + "-" * 120)
    print(f"{'任务名称':<20} {'实际时间':<10} {'计算时间':<10} {'Token消耗':<10} {'开始Token':<10} {'结束Token':<10} {'工具调用':<10} {'代码长度':<10} {'描述':<20}")
    print("-" * 120)
    
    for row in report_data:
        print(f"{row['任务名称']:<20} {row['实际执行时间(秒)']:<10} {row['计算执行时间(秒)']:<10} "
              f"{row['Token消耗']:<10} {row['开始Token']:<10} {row['结束Token']:<10} "
              f"{row['工具调用次数']:<10} {row['代码/指令长度']:<10} {row['描述']:<20}")
    ...
    if configA_tasks and configB_tasks:
        ...
        print(f"   Token节省比例: {(1 - configB_total_tokens/configA_total_tokens)*100:.1f}%" if configA_total_tokens > 0 else "N/A")
        print(f"   配置A平均时间: {configA_avg_time:.2f}秒")
        print(f"   配置B平均时间: {configB_avg_time:.2f}秒")
        print(f"   时间节省比例: {(1 - configB_avg_time/configA_avg_time)*100:.1f}%" if configA_avg_time > 0 else "N/A")
